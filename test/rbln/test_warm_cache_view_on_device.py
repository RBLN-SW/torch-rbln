...
def _assert_fp16_close(actual: torch.Tensor, expected: torch.Tensor) -> None:
    """rebel fp16 + view-on-device's ``contrib_dummy_cast`` (f16↔dlf16)
    round-trip introduces ~1 ULP drift per stage. Tolerance here is wide
    enough to absorb that drift while still catching the orders-of-magnitude
    error that a wrong-storage hit would produce."""
