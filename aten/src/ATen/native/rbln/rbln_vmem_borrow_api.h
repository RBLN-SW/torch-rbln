...
// but the device→host transfer is skipped even when the entry is physical-
// latest. Callers MUST overwrite the entire region before any consumer
// reads it; otherwise the host view will contain stale data. State
// transitions to USER_VIEW_IS_LATEST on return.
::rbln::Status rbln_v_acquire_host_ptr_for_overwrite(uint64_t vaddr, uint64_t size,
                                                     uintptr_t& host_ptr_out,
                                                     uint64_t& borrow_id_out);
