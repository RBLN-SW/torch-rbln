...
2. **Check the log or the counter, not the numbers.** With `TORCH_RBLN_LOG_LEVEL=INFO` every fallback prints ```aten::<op>` op ran on CPU instead of RBLN``; `_C._dispatch_fallback_by_op()` and `cpu_fallback_counts()` (`ops_utils.py`) give the same fact as a count a test can assert. No fallback on an aligned input is the proof. `TORCH_RBLN_DISABLE_FALLBACK=compile_error` does not cover the alignment route and will not raise for you.
