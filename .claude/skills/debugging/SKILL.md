...
Signals for layer 3: the same graph produces different results across compiler versions; an error code from the runtime; a failure that disappears with `TORCH_RBLN_*` knobs that only change how much work is handed to the compiler; a fault inside `librbln`. Confirm by installing a different rebel-compiler version and re-running — a version bisect is worth the time, because it converts a guess into a fact. `uv run tools/pin_rebel_compiler.py --current` prints the pinned build, and `uv.lock` names the index it came from. Install another with `uv pip install --no-config --index <url> rebel-compiler==<version>`: the pin would otherwise refuse the version, and `--no-config` drops the index list too. Do not commit the pin change. Building against a compiler source checkout, when you need to test a candidate fix, swaps this repository's `pyproject.toml` and `uv.lock`, then rebuilds the venv — ask first and use a throwaway worktree.
