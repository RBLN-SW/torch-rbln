---
name: writing-tests
description: Use when adding or modifying tests under test/ — where the file goes, the markers that decide whether it runs at all, the process-state traps in this suite, and how to prove a new test fails before the fix.
---
...
- **A faulted device poisons the rest of the worker.** If your test drains with in-flight work, the remaining tests on that worker skip. Do not paper over it with a retry.
