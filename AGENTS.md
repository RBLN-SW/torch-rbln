...
- **Do not commit a version change to test one.** A workflow owns `constraints-build-dev.txt` and opens its own PR against it (`docs/WORKFLOWS.md`), so a hand edit on a feature branch collides with that. Install the version you want with a direct spec, or the pinned one with `--constraint constraints-build-dev.txt`. If a tool rewrote `pyproject.toml` to point at a local build, revert that before committing. Moving the range in `pyproject.toml` is a change of its own — `docs/THIRD_PARTY_UPDATE.md` owns the procedure and the two places that stay aligned.
...
- Reuse before you write: no new file when the code fits in one that exists, no helper called once, no abstraction for a single use. Delete your scratch files.
