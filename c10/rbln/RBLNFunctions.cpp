...
void memcpy_v2v_multi(const std::vector<V2VCopyOp>& copies) {
  ...
  for (const auto& c : copies) {
    RBLN_CHECK(c.nbytes > 0, "nbytes must be positive");
    RBLN_CHECK(c.src != nullptr, "rbln_src_data cannot be nullptr");
    RBLN_CHECK(c.dst != nullptr, "rbln_dst_data cannot be nullptr");
    bump_v2v(c.nbytes);
    rbln_copies.emplace_back(reinterpret_cast<uint64_t>(c.src),
                             reinterpret_cast<uint64_t>(c.dst),
                             static_cast<uint64_t>(c.nbytes));
  }
